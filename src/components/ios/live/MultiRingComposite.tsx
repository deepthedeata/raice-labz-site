...
interface Props {
  /**
   * Rings in outer→inner order. The first index is the outermost. Supports
   * 2–5 rings; radii are computed to fit inside `size`.
   */
  rings: RingDef[];
  size?: number;
  /** Initially-featured ring index (centre text). */
  initialFeatured?: number;
  className?: string;
}
...
export function MultiRingComposite({
  ...
  // Auto-tune stroke + gap so any 2–6 rings fit comfortably inside `size`.
  // 3 rings → 16/6 (the original look); more rings → thinner strokes.
  const count = rings.length;
  const stroke = count <= 3 ? 16 : count === 4 ? 13 : count === 5 ? 11 : 9;
  const gap = count <= 3 ? 6 : count === 4 ? 5 : 4;
  // Outer ring radius then inset by (stroke + gap) per inner ring.
  const outerR = (size - stroke) / 2;
  const radii = Array.from({ length: count }, (_, i) => outerR - i * (stroke + gap));
