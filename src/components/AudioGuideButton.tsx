...
// Persisted app-wide: once the user pauses the guide, it stays silent across every
// subsequent section/page/tab (and reloads) until they press Replay again.
const AUDIO_GUIDE_ENABLED_KEY = "raice-audio-guide-enabled";

function isAudioGuideEnabled(): boolean {
  if (typeof window === "undefined") return true;
  return localStorage.getItem(AUDIO_GUIDE_ENABLED_KEY) !== "false";
}

function setAudioGuideEnabled(enabled: boolean) {
  if (typeof window === "undefined") return;
  localStorage.setItem(AUDIO_GUIDE_ENABLED_KEY, String(enabled));
}
...
export const AudioGuideButton = ({ clipId }: AudioGuideButtonProps) => {
  ...
  useEffect(() => {
    if (!clipId) return;
    if (!isAudioGuideEnabled()) return; // user disabled the guide — stay silent in every section until re-enabled
