...
const replayIconForMode = (modeId?: string, modeType?: string) => {
  ...
  if (id.startsWith("PROC") || id.startsWith("PRT") || id.startsWith("IND") || modeType === "procurement") {
    return <img src="/machineimages/procurement.png" alt="" className="w-full h-full object-cover" />;
  }
  if (id.startsWith("PROD") || modeType === "production") {
    return <img src="/machineimages/production.png" alt="" className="w-full h-full object-cover" />;
  }
  if (id.startsWith("MILL") || id.startsWith("MR") || modeType === "milled-rice") {
    return <img src="/machineimages/milled-rice.png" alt="" className="w-full h-full object-cover" />;
  }
...
function ReplayCard({ run }: { run: any }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="w-[4.5rem] h-[4.5rem] rounded-[18px] overflow-hidden flex items-center justify-center shadow-lg bg-white/10">
          {icon}
        </div>
        <div
      ...
      <Tile padded={false} className="overflow-hidden h-full">
        <div className="p-5">
          ...
          <div className="grid grid-cols-1 gap-2">
            <PrimaryAction onLaunch={onLaunch} to="/procurement-analysis" label={t('dashboard.procurement')} sub={t('dashboard.rawPaddy')} icon={<img src="/machineimages/procurement.png" alt="" className="w-full h-full object-cover" />} color="hsl(var(--ios-green))" />
            <PrimaryAction onLaunch={onLaunch} to="/production-analysis" label={t('dashboard.production')} sub={t('dashboard.machineWise')} icon={<img src="/machineimages/production.png" alt="" className="w-full h-full object-cover" />} color="hsl(var(--accent))" />
            <PrimaryAction onLaunch={onLaunch} to="/milled-rice-analysis" label={t('dashboard.milledRice')} sub={t('dashboard.finalStage')} icon={<img src="/machineimages/milled-rice.png" alt="" className="w-full h-full object-cover" />} color="hsl(var(--ios-orange))" />
          </div>
          <div className="mt-6 pt-6 border-t border-gray-200/30">
            <div className="grid grid-cols-2 gap-2">
              <DisabledAnalysisCard title={t('dashboard.cookedRiceQuality')} sub={t('dashboard.cookingProperties')} icon={<CookingPot className="w-6 h-6" />} color="hsl(var(--ios-text-tertiary))" />
              <DisabledAnalysisCard title={t('dashboard.predictiveAnalysis')} sub={t('dashboard.yieldForecasting')} icon={<TrendingUp className="w-6 h-6" />} color="hsl(var(--ios-text-tertiary))" />
            </div>
          </div>
        </div>
      </Tile>
...
function PrimaryAction({
  ...
  return (
    ...
    >
      <div
        className="w-10 h-10 rounded-[8px] overflow-hidden flex items-center justify-center text-white"
...
function DisabledAnalysisCard({
  ...
  return (
    ...
    >
      <div
        className="w-10 h-10 rounded-[8px] flex items-center justify-center text-white opacity-70"
        style={{ background: color }}
      >
        {icon}
      </div>
