...
const DataReports = () => {
  ...
  const fetchProcessData = async () => {
    ...
    try {
      ...
      // Map analysisType to the backend viewMode
      const getViewMode = () => {
        switch (analysisType) {
      ...
      if (analysisType !== 'all') baseParams.analysisType = analysisType;
      if (varietyFilter !== 'all') baseParams.variety = varietyFilter;
      if (seasonFilter !== 'all') baseParams.season = seasonFilter;
      // For single-machine filter, pass machineName to grain-analysis
      if (isMachine) baseParams.machineName = filterValue;

      // Determine which endpoints to fetch based on filter selection
      // series: -> only TMA (series), machine: -> only grain-analysis (single machine), all -> both
      const isProduction = analysisType === 'production';
      const fetchGrainAnalysis = !isProduction || !isSeries;  // skip grain-analysis only when filtering by series
      const fetchTmaAnalysis = analysisType === 'all' || (isProduction && !isMachine);  // skip TMA when filtering by machine

      const fetches: Promise<any[]>[] = [];

      // grain-analysis endpoint (single machine production, procurement, milled-rice, etc.)
      if (fetchGrainAnalysis) {
        const requestBody = { ...baseParams, viewMode: getViewMode() };
        fetches.push(
      ...
      if (fetchTmaAnalysis) {
        ...
        // Pass variety filter so backend can restrict to matching modes
        if (varietyFilter !== 'all') tmaBody.variety = varietyFilter;
        if (seasonFilter !== 'all') tmaBody.season = seasonFilter;
        fetches.push(
  ...
  const handleDownloadReport = async (process: ProcessData) => {
    ...
    try {
      ...
      // Map analysisType to report_type for download
      let reportType = 'individual';
      if (analysisType === 'procurement') {
        reportType = 'procurement';
      } else if (analysisType === 'production') {
        // Series mode uses TMA report, single machine uses production report
        reportType = entryModeType === 'tma' ? 'tma' : 'production';
      } else if (analysisType === 'milled-rice') {
        reportType = 'milled-rice';
      } else {
        // "all" — determine from modeId prefix + modeType
        if (modeId.startsWith('PRT-')) reportType = 'procurement';
        else if (modeId.startsWith('PROD-')) reportType = 'production';
        else if (modeId.startsWith('MR-')) reportType = 'milled-rice';
        else if (modeId.startsWith('BAT-')) reportType = 'batch';
        else if (modeId.startsWith('MACH-')) reportType = 'machine';
        else if (modeId.startsWith('IND-')) reportType = 'individual';
      }
      ...
      if (response.ok) {
        ...
        a.href = url;
        const contentDisposition = response.headers.get("Content-Disposition");
        const filenameReportType = getFilenameReportType(modeId, analysisType, entryModeType);
        ...
        toast({
          ...
          description: `${getReportTypeConfig(analysisType).label} report for ${processName} has been downloaded successfully.`,
        });
      } else {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to generate report');
  ...
  }, [fromDate, toDate, analysisType, varietyFilter, seasonFilter, machineFilter, productionMode]);
  ...
  const getAnalysisBadge = (modeId: string) => {
    const type = analysisType !== 'all' ? analysisType : getAnalysisTypeFromModeId(modeId);
    switch (type) {
      case 'procurement':
  ...
  const handleViewDetailedGrains = (process: ProcessData) => {
    ...
    const getModeFromAnalysisType = () => {
      const isTma = process.modeType === 'tma';
      if (analysisType === 'production') return isTma ? 'TMA' : 'Machine-wise';
      if (analysisType === 'all') {
        if (modeId.startsWith('PROD-')) return isTma ? 'TMA' : 'Machine-wise';
    ...
    const mode = getModeFromAnalysisType();
        ...
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2">
                  ...
                  <RadioGroup value={analysisType} onValueChange={(val) => { setAnalysisType(val); if (val !== 'production') { setMachineFilter('all'); setProductionMode('all'); } }} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center space-x-3 p-4 border rounded-lg hover:bg-gray-50 cursor-pointer">
                      <RadioGroupItem value="procurement" id="rpt-procurement" />
                      <Label htmlFor="rpt-procurement" className="cursor-pointer flex items-center space-x-3 flex-1">
                        <ShoppingCart className="w-5 h-5 text-emerald-600" />
                        <div>
                          <div className="font-semibold">Procurement</div>
                          <div className="text-sm text-gray-600">Procurement analysis reports</div>
                        </div>
                      </Label>
                    </div>

                    <div className="flex items-center space-x-3 p-4 border rounded-lg hover:bg-gray-50 cursor-pointer">
                      <RadioGroupItem value="production" id="rpt-production" />
                      <Label htmlFor="rpt-production" className="cursor-pointer flex items-center space-x-3 flex-1">
                        <Factory className="w-5 h-5 text-blue-600" />
                        <div>
                          <div className="font-semibold">Production</div>
                          <div className="text-sm text-gray-600">Production analysis reports</div>
                        </div>
                      </Label>
                    </div>

                    <div className="flex items-center space-x-3 p-4 border rounded-lg hover:bg-gray-50 cursor-pointer">
                      <RadioGroupItem value="milled-rice" id="rpt-milled-rice" />
                      <Label htmlFor="rpt-milled-rice" className="cursor-pointer flex items-center space-x-3 flex-1">
                        <Wheat className="w-5 h-5 text-amber-600" />
                        <div>
                          <div className="font-semibold">Milled Rice Quality</div>
                          <div className="text-sm text-gray-600">Milled rice quality reports</div>
                        </div>
                      </Label>
                    </div>

                    <div className="flex items-center space-x-3 p-4 border rounded-lg border-dashed opacity-50 pointer-events-none">
                      <RadioGroupItem value="cooked-rice" id="rpt-cooked-rice" disabled />
                      <Label htmlFor="rpt-cooked-rice" className="flex items-center space-x-3 flex-1">
              ...
              <div className={`grid grid-cols-1 md:grid-cols-2 ${analysisType === 'production' ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-4 mt-6 pt-4 border-t`}>
                <div className="space-y-2">
          ...
          <Card className="animate-fade-in" style={{ animationDelay: "100ms" }}>
            <CardHeader>
              <CardTitle className="flex items-center space-x-2 text-rice-primary">
                {(() => {
                  const config = getReportTypeConfig(analysisType);
            ...
            <CardContent>
              <div className="space-y-6">
                {!fromDate || !toDate ? (
                  ...
                  <div className="text-center py-8 text-gray-500">
                    {fromDate && toDate
                      ? `No ${getReportTypeConfig(analysisType).label.toLowerCase()} found for the selected date range (${format(fromDate, "MMM dd, yyyy")} - ${format(toDate, "MMM dd, yyyy")}).`
                      : `No ${getReportTypeConfig(analysisType).label.toLowerCase()} available. Select a date range to view sessions.`
                    }
                  </div>
                ) : (
