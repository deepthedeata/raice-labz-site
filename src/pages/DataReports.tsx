...
import { CalendarIcon, Download, Package, User, Settings, Microscope, Eye, X, Wheat, FileText, Filter } from "lucide-react";
import { PaddyGrainIcon, RiceProcessIcon } from "@/components/icons/RiceIcons";
import { ToggleGroup, ToggleGroupItem } from "@/components/ui/toggle-group";
import { ReportsAnalyticsView } from "@/components/reports/ReportsAnalyticsView";
import type { AnalyticsDomain } from "@/lib/reportsAnalytics";
...
const DataReports = () => {
  ...
  const [reportsViewMode, setReportsViewMode] = useState<"reports" | "analytics">("reports");
            ...
            lines.forEach((line: { name?: string; machines?: (string | { name?: string; machineNumber?: string })[] }) => {
              ...
              if (Array.isArray(line.machines)) {
                line.machines.forEach((m) => {
                  ...
                  if (displayName) lineMachineNames.push(displayName);
                });
              }
              if (line.name) perLine.push({ name: line.name, machines: lineMachineNames });
            });
            setLineMachines(allMachines);
            setLineNames(allLineNames);
            setMillLines(perLine);
          }
        }
      } catch (error) {
  ...
  const getReportTypeConfig = (type: string) => {
    switch (type) {
      case "procurement":
        return { icon: PaddyGrainIcon, label: t('dataReports.procurementReportsTitle'), description: t('dataReports.procurementDesc'), color: "text-emerald-600" };
      case "production":
        return { icon: RiceProcessIcon, label: t('dataReports.productionReportsTitle'), description: t('dataReports.productionDesc'), color: "text-blue-600" };
      case "milled-rice":
  ...
  // Tag each filtered process with its analytics domain, reusing the same
  // classification already used by getAnalysisBadge, for the Analytics view.
  const analyticsProcesses = useMemo(() => filteredProcesses.map((p) => ({
    ...p,
    domain: (p.modeType === 'tma' ? 'production' : (p.modeType || getAnalysisTypeFromModeId(p.id.split('_')[0]))) as AnalyticsDomain,
  })), [filteredProcesses]);
        ...
        <div className="max-w-6xl mx-auto space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      { value: 'procurement', label: t('dataReports.procurement'), description: t('dataReports.procurementDesc'), icon: PaddyGrainIcon, color: 'text-emerald-600' },
                      { value: 'production', label: t('dataReports.production'), description: t('dataReports.productionDesc'), icon: RiceProcessIcon, color: 'text-blue-600' },
          ...
          <Card className="animate-fade-in" style={{ animationDelay: "100ms" }}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 gap-4">
              ...
              <ToggleGroup
                type="single"
                value={reportsViewMode}
                onValueChange={(v) => v && setReportsViewMode(v as "reports" | "analytics")}
              >
                <ToggleGroupItem value="reports" className="text-xs px-3">
                  Reports
                </ToggleGroupItem>
                <ToggleGroupItem value="analytics" className="text-xs px-3">
                  Analytics
                </ToggleGroupItem>
              </ToggleGroup>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                ...
                ) : reportsViewMode === "analytics" ? (
                  <ReportsAnalyticsView processes={analyticsProcesses} selectedAnalysisTypes={selectedAnalysisTypes} lineNames={lineNames} lineMachines={lineMachines} millLines={millLines} />
                ) : (
