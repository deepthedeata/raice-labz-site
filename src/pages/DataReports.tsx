...
const DataReports = () => {
  ...
  const fetchProcessData = async () => {
    ...
    try {
      ...
      if (machineFilter !== 'all') baseParams.machineName = machineFilter;

      // For production or "all", we need both grain-analysis (single machine)
      // AND tma-analysis (series) endpoints, then merge results.
      const needGrainAnalysis = analysisType !== 'production' || analysisType === 'all';
      const needTmaAnalysis = analysisType === 'production' || analysisType === 'all';
      ...
      if (analysisType === 'production' || analysisType === 'all') {
        const tmaBody = { fromDate: fromDateStr, toDate: toDateStr };
        fetches.push(
  ...
  const handleDownloadReport = async (processId: string, processName: string) => {
    ...
    try {
      ...
      const entryModeType = processEntry?.modeType || '';
      ...
      if (response.ok) {
        ...
        const contentDisposition = response.headers.get('Content-Disposition');
        let filename = `${modeId}_${reportType}_report.pdf`;
        if (contentDisposition) {
  ...
  useEffect(() => {
    ...
    const fetchFilterOptions = async () => {
      try {
        const response = await fetch(`${BACKEND_URL}/api/raice_labz/analytics/filter-options`);
        if (response.ok) {
          const data = await response.json();
          setFilterOptions({
  ...
  }, [fromDate, toDate, analysisType, varietyFilter, seasonFilter, machineFilter]);
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2">
                  ...
                  <RadioGroup value={analysisType} onValueChange={setAnalysisType} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Additional Filters: Variety + Season + Machine + Clear */}
