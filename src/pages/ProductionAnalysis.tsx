...
const ProductionAnalysis = () => {
  ...
  const { setSeriesExecutionLocked } = useAnalysis();
  ...
  useEffect(() => {
    ...
    const loadMachines = async () => {
      ...
      try {
        ...
        // Build machine list: collect unique machines from all series lines + top-level machines
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/generate-report`, {
        ...
        body: JSON.stringify({ mode_id: modeId, report_type: "production" }),
      });
      if (!res.ok) {
              ...
              <Card className="h-full">
                ...
                <CardContent className="space-y-4 text-sm text-gray-700">
                  <div className="space-y-1">
                    <Label htmlFor="prod-rice-mill">Rice Mill <span className="text-rice-primary">*</span></Label>
                    <Input
                      id="prod-rice-mill"
                      value={riceMill}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <Label htmlFor="prod-operator-name">Operator Name <span className="text-rice-primary">*</span></Label>
                      <Input
                        id="prod-operator-name"
                        value={operatorName}
                    ...
                    <div className="space-y-1">
                      <Label htmlFor="prod-location">Location <span className="text-rice-primary">*</span></Label>
                    ...
                    <div className="space-y-1">
                      <Label htmlFor="prod-test-date">Date (IST) <span className="text-rice-primary">*</span></Label>
                      <Input
                        id="prod-test-date"
                        value={currentDate}
                        readOnly
                    ...
                    <div className="space-y-1">
                      <Label htmlFor="prod-test-time">Time (IST) <span className="text-rice-primary">*</span></Label>
                      <Input
                        id="prod-test-time"
                        value={currentTime}
                        readOnly
                      ...
                      <div className="flex flex-wrap items-center gap-4">
                        ...
                        >
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem
                              value="50 grams"
                              id="prod-sample-weight-50"
                            />
                            <Label
                              htmlFor="prod-sample-weight-50"
                          ...
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem
                              value="100 grams"
                              id="prod-sample-weight-100"
                            />
                            <Label
                              htmlFor="prod-sample-weight-100"
                              className="cursor-pointer font-normal"
                            >
                              100 grams
                            </Label>
                          ...
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem
                              value="free weight"
                              id="prod-sample-weight-free"
                            />
                            <Label
                              htmlFor="prod-sample-weight-free"
                              className="cursor-pointer font-normal"
                            >
                              Free weight
                            </Label>
                        ...
                        {sampleWeight === "free weight" && (
                          <div className="flex items-center gap-2">
                            <Input
                              ...
                              onChange={(e) =>
                                setFreeWeightInput(e.target.value)
                              }
                              className="w-28"
                            />
          ...
          <TabsContent value="live" className="mt-2">
            <ProcurementLiveAnalysis
              ...
              analysisDataOverride={{
                ...
                ...(isSeriesMode
                  ? { machines: [selectedMachine || currentSeriesMachine] }
                  : { machineIndex: undefined }),
              }}
              onComplete={() => {
                if (isSeriesMode) {
                  const justCompleted = selectedMachine || currentSeriesMachine;
                  const newCompleted = [...completedSeriesMachines, justCompleted];
                  setCompletedSeriesMachines(newCompleted);

                  const modeId = sessionStorage.getItem("production_series_mode_id") || seriesModeId;
                  if (modeId) {
                    setSeriesModeId(modeId);
                  }

                  // Call advance-machine to mark this machine's trials as completed
                  if (modeId) {
                    const machineIndex = displayedMachines.indexOf(justCompleted);
                    fetch(`${BACKEND_URL}/api/raice_labz/modes/${modeId}/advance-machine`, {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ machineName: justCompleted, machineIndex }),
                    })
                      .then((res) => res.json())
                      .then((data) => {
                        if (data.allComplete) {
                          // ALL machines done — wait for backend metrics flush before navigating to reports
                          toast({
                            title: "Series Complete",
                            description: `All ${displayedMachines.length} machines in "${selectedSeries}" analyzed. Finalizing metrics...`,
                            duration: 5000,
                          });
                          // The backend stop-trial does a 20s delayed flush in a background thread.
                          // Wait a bit before navigating to reports; the report fetch will also poll/retry.
                          setTimeout(() => markStepComplete("live"), 3000);
                        } else {
                          // More machines remain - auto-advance to next machine
                          const nextMachineIdx = displayedMachines.indexOf(justCompleted) + 1;
                          const nextMachine = displayedMachines[nextMachineIdx] || "";
                          if (nextMachine) {
                            setSelectedMachine(nextMachine);
                            toast({
                              title: `Advancing to ${nextMachine}`,
                              description: `Machine ${nextMachineIdx + 1} of ${displayedMachines.length}. Auto-starting in 3 seconds...`,
                              duration: 3000,
                            });
                          }
                        }
                      ...
                      .catch((err) => {
                        console.error("advance-machine error:", err);
                        setJustCompletedMachine(justCompleted);
                        setShowPostMachineDialog(true);
                      });
                  }
                } else {
                  // Single-machine mode
                  ...
                  <div className="flex items-center gap-2 flex-wrap pl-2 border-l-2 border-rice-primary/30">
                    {currentMachineData.trials.map((trial, idx) => (
                      ...
                      >
                        ...
                        {trial.sessionStatus === "completed" && <CheckCircle2 className="w-3 h-3 inline ml-1 text-green-500" />}
                      </button>
                    ))}
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-gradient-to-r from-gray-50 to-white border-b">
                              ...
                              <th className="text-right py-2.5 px-3 font-semibold text-green-600">Head Rice %</th>
                              <th className="text-right py-2.5 px-3 font-semibold text-amber-600">Broken %</th>
                              <th className="text-right py-2.5 px-3 font-semibold text-orange-600">Chalky %</th>
                              <th className="text-right py-2.5 px-3 font-semibold text-red-600">FM %</th>
                            ...
                            {detailedReport.machines.map((machine, idx) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="py-2.5 px-3 text-right tabular-nums font-semibold text-green-700">{params.headRicePct}%</td>
                                  <td className="py-2.5 px-3 text-right tabular-nums text-amber-700">{params.brokenRicePct}%</td>
                                  <td className="py-2.5 px-3 text-right tabular-nums text-orange-700">{params.chalkyPct}%</td>
                                  <td className="py-2.5 px-3 text-right tabular-nums text-red-700">{params.foreignMatterPct}%</td>
                                  <td className="py-2.5 px-3 text-center">
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                                      machine.status === "completed" ? "bg-green-100 text-green-700"
                                        : machine.status === "pending" ? "bg-gray-100 text-gray-500"
                                        : "bg-yellow-100 text-yellow-700"
                                    }`}>
                                      {machine.status === "pending" ? "Not analyzed" : machine.status}
                                    </span>
                                  </td>
                                </tr>
                              );
