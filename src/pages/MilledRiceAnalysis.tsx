...
const BASMATI_CLASSIFICATION_METRICS = [
  { key: "headRice", label: "Head Rice", unit: "%", border: "border-green-200", bg: "bg-gradient-to-br from-green-50 to-emerald-50", textColor: "text-green-700", barColor: "bg-green-500" },
  { key: "totalBroken", label: "Broken Rice", unit: "%", border: "border-amber-200", bg: "bg-gradient-to-br from-amber-50 to-yellow-50", textColor: "text-amber-700", barColor: "bg-amber-500" },
  { key: "chalky", label: "Chalky", unit: "%", border: "border-orange-200", bg: "bg-gradient-to-br from-orange-50 to-amber-50", textColor: "text-orange-700", barColor: "bg-orange-500" },
  { key: "discoloured", label: "Discoloured", unit: "%", border: "border-purple-200", bg: "bg-gradient-to-br from-purple-50 to-violet-50", textColor: "text-purple-700", barColor: "bg-purple-500" },
];
  ...
  const markStepComplete = async (step: StepId) => {
    if (step === "preparation") {
      ...
      try {
        ...
        sessionStorage.setItem("milled_rice_sampling_point", samplingPoint);
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
        <Tabs value={activeStep} onValueChange={(v) => handleStepChange(v as StepId)} className="w-full">
          ...
          <TabsContent value="preparation" className="mt-2">
            {/* Category Banner */}
            <div
              className={`flex items-center justify-between rounded-lg px-4 py-2 mb-6 ${
                category === "basmati"
            ...
            >
              <div className="flex items-center gap-2">
                <Wheat
                  className={`w-5 h-5 ${
                    category === "basmati" ? "text-green-600" : "text-amber-600"
                  }`}
                />
                <span
                  className={`font-semibold text-sm ${
                    category === "basmati" ? "text-green-700" : "text-amber-700"
                  }`}
                >
              ...
              <Card className="h-full">
                ...
                <CardContent className="space-y-4 text-sm text-gray-700">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      <Label htmlFor="grain-sampling-point">Sampling Point <span className="text-rice-primary">*</span></Label>
                      <Select
                        value={samplingPoint || ""}
                        onValueChange={setSamplingPoint}
                      >
                        <SelectTrigger id="grain-sampling-point">
                          <SelectValue placeholder="Select sampling point" />
                        </SelectTrigger>
                        <SelectContent>
                          {SAMPLING_POINTS.map((sp) => (
                      ...
                      <div className="flex flex-wrap items-center gap-4">
                        ...
                        >
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem value="50 grams" id="mr-sample-weight-50" />
                            <Label htmlFor="mr-sample-weight-50" className="cursor-pointer font-normal">
                              50 grams
                            </Label>
                          </div>
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem value="100 grams" id="mr-sample-weight-100" />
                            <Label htmlFor="mr-sample-weight-100" className="cursor-pointer font-normal">
                              100 grams
                            </Label>
                          </div>
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem value="free weight" id="mr-sample-weight-free" />
                            <Label htmlFor="mr-sample-weight-free" className="cursor-pointer font-normal">
                              Free weight
                            </Label>
                          </div>
                        </RadioGroup>
          ...
          <TabsContent value="live" className="mt-2">
            {/* Category-colored scanning banner */}
            <div
              className={`flex items-center justify-center rounded-lg px-4 py-3 mb-4 ${
                category === "basmati"
                  ? "bg-green-50 border border-green-200"
                  : "bg-amber-50 border border-amber-200"
              }`}
              ...
              <div className="space-y-4">
                ...
                <Card className="shadow-sm">
                  ...
                  <CardContent className="pt-5">
                    <div className={`grid grid-cols-2 ${qualityIndicators.length === 4 ? "sm:grid-cols-4" : "sm:grid-cols-2"} gap-3`}>
                      {qualityIndicators.map((metric) => {
                        const comingSoonKeys = ["dom", "don", "translucency", "moisture"];
                        const isComingSoon = comingSoonKeys.includes(metric.key);
                        const val = metric.key === "wi" ? (currentOutputParams?.whitenessIndex ?? "--") : isComingSoon ? null : "--";
                        return (
                        <div
                          key={metric.key}
                        ...
                        >
                          ...
                          {isComingSoon ? (
                            <p className="text-sm font-semibold text-gray-400 mt-2">Soon</p>
                          ) : (
                            <p className={`text-2xl font-bold ${metric.textColor} mt-1`}>
                              {val}{metric.unit && <span className="text-sm font-normal ml-0.5 text-gray-400">{metric.unit}</span>}
                            </p>
                          )}
                        </div>
                        );
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {classificationMetrics.map((metric) => {
                        const keyMap: Record<string, number | undefined> = {
                          headRice: currentOutputParams?.headRicePct,
                          totalBroken: currentOutputParams?.brokenRicePct,
                          chalky: currentOutputParams?.chalkyPct,
                          discoloured: currentOutputParams?.discolouredPct,
                        };
                        const val = keyMap[metric.key];
                        ...
                        >
                          <p className="text-[11px] font-medium text-gray-500 group-hover:text-gray-700 transition-colors duration-300">{metric.label}</p>
                          <p className={`text-2xl font-bold ${metric.textColor} mt-1`}>
                            {display}<span className="text-sm font-normal ml-0.5 text-gray-400">%</span>
                          </p>
                          <div className="mt-2 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${metric.barColor} transition-all duration-700`} style={{ width: `${typeof val === 'number' ? Math.min(val, 100) : 0}%` }} />
                          </div>
                        </div>
                        );
                      })}
