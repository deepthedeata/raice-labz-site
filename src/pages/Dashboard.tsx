...
import { PaddyGrainIcon, RiceProcessIcon, GlossyRiceIcon } from "@/components/icons/RiceIcons";
...
const Dashboard = () => {
  ...
  const animatedGrainsPerSecond = useAnimatedNumber(analyticsData?.peak_grains_per_second ?? 0, { duration: 600 });
  useEffect(() => {
    fetchAnalyticsData();
  ...
  const analysisTiles = [
    {
      ...
      icon: PaddyGrainIcon,
      image: "/machineimages/procurement.png",
    ...
    },
    {
      ...
      icon: RiceProcessIcon,
      image: "/machineimages/production.png",
    ...
    },
    {
      ...
      icon: GlossyRiceIcon,
      image: "/machineimages/milled-rice.png",
              ...
              {analysisTiles.map((tile, index) => {
                ...
                const inner = (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`w-16 h-16 rounded-xl overflow-hidden flex items-center justify-center shadow-lg transition-all duration-300 ${
                          tile.image ? "" : `bg-gradient-to-br ${tile.color}`
                        } ${tile.disabled ? "" : "group-hover:shadow-xl group-hover:scale-110"}`}
                      >
                        {tile.image ? (
                          <img src={tile.image} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <TileIcon className="w-10 h-10 text-white" />
                        )}
                      </div>
