...
import { Camera, Save, Trash2, Play, Pause, Square, BarChart3, Percent, CheckCircle, Circle, Settings, AlertCircle, Loader2, Clock, ZoomIn, ZoomOut, Maximize, Minimize, Factory } from "lucide-react";
...
const ProcurementLiveAnalysis = ({ embedded = false, analysisDataOverride, onComplete }: LiveAnalysisProps = {}) => {
  ...
  }, [analysisDataOverride?.machineName]);
  ...
  const hideBrownRice = analysisData?.analysisType === 'procurement'
    || ['tray separator', 'tsep', 'paddy sort', 'husker', 'shell', 'whitener', 'white', 'silky', 'polisher', 'bright', 'color sorter', 'colour sorter', 'sortex', 'intel vision', 'length grader', 'length sort', 'sifter', 'sift', 'shifter', 'blend', 'pack', 'thickness grader', 'thickthin', 'thick thin'].some(k => currentMachineName.includes(k));
  ...
  const buildMetricsArray = (displayMetrics: typeof initialMetrics, displayCounts: typeof zeroCounts) => {
    return [
      ...
      { category: 'Rejections', name: 'Chalky', value: displayMetrics.rejections.harvest.chalkyBellyCore, count: displayCounts.rejections.harvest.chalkyBellyCore, color: 'bg-red-500', textColor: 'text-red-700' },
      { category: 'Rejections', name: 'Yellow', value: displayMetrics.rejections.harvest.yellow, count: displayCounts.rejections.harvest.yellow, color: 'bg-red-400', textColor: 'text-red-700' },
      // { category: 'Rejections', name: 'Black', value: displayMetrics.rejections.harvest.black, count: displayCounts.rejections.harvest.black, color: 'bg-red-300', textColor: 'text-red-700' },
      { category: 'Rejections', name: 'Immature (Green)', value: displayMetrics.rejections.harvest.immatureGreen, count: displayCounts.rejections.harvest.immatureGreen, color: 'bg-red-200', textColor: 'text-red-700' },
      { category: 'Rejections', name: 'Pecky grains', value: displayMetrics.rejections.harvest.peckyGrains, count: displayCounts.rejections.harvest.peckyGrains, color: 'bg-red-100', textColor: 'text-red-700' },
      { category: 'Rejections', name: 'Discolored', value: displayMetrics.rejections.harvest.discolored, count: displayCounts.rejections.harvest.discolored, color: 'bg-red-50', textColor: 'text-red-700' },
      ...
      // Foreign Matter (flattened - no organic/inorganic subdivisions)
      ...
      { category: 'Foreign Matter', name: 'Chaff', value: displayMetrics.foreignMatter.organic.chaff, count: displayCounts.foreignMatter.organic.chaff, color: 'bg-orange-200', textColor: 'text-orange-700' },
      { category: 'Foreign Matter', name: 'Straw', value: displayMetrics.foreignMatter.organic.straw, count: displayCounts.foreignMatter.organic.straw, color: 'bg-orange-100', textColor: 'text-orange-700' },
      // { category: 'Foreign Matter', name: 'Sticks', value: displayMetrics.foreignMatter.organic.sticks, count: displayCounts.foreignMatter.organic.sticks, color: 'bg-orange-50', textColor: 'text-orange-700' },
      // Brown rice: hide for tray separator/husker (brownrice is reclassified into goodRice subs)
      ...(!hideBrownRice ? [{ category: 'Foreign Matter', name: 'Brown rice', value: displayMetrics.foreignMatter.organic.brownRice, count: displayCounts.foreignMatter.organic.brownRice, color: 'bg-orange-100', textColor: 'text-orange-700' }] : []),
      { category: 'Foreign Matter', name: 'Stones', value: displayMetrics.foreignMatter.inorganic.stones, count: displayCounts.foreignMatter.inorganic.stones, color: 'bg-orange-500', textColor: 'text-orange-700' },
      // { category: 'Foreign Matter', name: 'Mud', value: displayMetrics.foreignMatter.inorganic.mud, count: displayCounts.foreignMatter.inorganic.mud, color: 'bg-orange-400', textColor: 'text-orange-700' },
      // { category: 'Foreign Matter', name: 'Thread', value: displayMetrics.foreignMatter.inorganic.thread, count: displayCounts.foreignMatter.inorganic.thread, color: 'bg-orange-300', textColor: 'text-orange-700' },
      { category: 'Foreign Matter', name: 'Plastic', value: displayMetrics.foreignMatter.inorganic.plastic, count: displayCounts.foreignMatter.inorganic.plastic, color: 'bg-orange-200', textColor: 'text-orange-700' },
      { category: 'Foreign Matter', name: 'Metals', value: displayMetrics.foreignMatter.inorganic.metals, count: displayCounts.foreignMatter.inorganic.metals, color: 'bg-orange-100', textColor: 'text-orange-700' },
      { category: 'Foreign Matter', name: 'Glass', value: displayMetrics.foreignMatter.inorganic.glass, count: displayCounts.foreignMatter.inorganic.glass, color: 'bg-orange-50', textColor: 'text-orange-700' },
      { category: 'Foreign Matter', name: 'Paper', value: displayMetrics.foreignMatter.inorganic.paper, count: displayCounts.foreignMatter.inorganic.paper, color: 'bg-orange-50', textColor: 'text-orange-700' },
      { category: 'Foreign Matter', name: 'Cardboard', value: displayMetrics.foreignMatter.inorganic.cardboard, count: displayCounts.foreignMatter.inorganic.cardboard, color: 'bg-orange-50', textColor: 'text-orange-700' },
  ...
  const stopAnalysis = async () => {
    ...
    try {
      ...
      // Automatically complete the sample when stopped
    ...
    if (currentSample < totalSamplesCount) {
      ...
      if (isTmaEnabled()) {
        ...
        setCompletedMachines(prev => {
          if (!prev.includes(currentMachine)) {
            console.log(`✅ Marking machine ${currentMachine} as completed`);
            return [...prev, currentMachine];
          }
          return prev;
        });
        
        // Move to next machine if available
        if (currentMachineIndex < machines.length - 1) {
          ...
          setCurrentSample(1); // Reset to sample 1 for new machine
          setCompletedSamples([]); // Reset completed samples for new machine
          ...
          console.log("🎉 All machines completed!");
          toast({
            title: "All Machines Completed",
            description: "TMA analysis is complete!",
          });

          // For production series, auto-signal machine completion to parent
          if (analysisData?.isProductionSeries) {
            console.log('🔄 Production series: auto-completing machine, signaling parent...');
            setTimeout(() => {
              onComplete?.();
            }, 1000);
          }
        ...
        <div className={embedded ? "w-full space-y-6" : "max-w-7xl mx-auto space-y-6"}>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between pt-4 mt-4 border-t">
                      <div className="flex items-center space-x-4 flex-wrap">
                        <Button
                          ...
                          disabled={(isStartLoading || isStopLoading) || (!isContainerReady && !isAnalysisRunning) || (!isGrainDetailsFilled || (!canStartCurrentSample && !isAnalysisRunning))}
                          className={`${
                            isAnalysisRunning
                        ...
                        {/* Timer */}
                        <div className="flex items-center space-x-2 px-3 py-2 bg-white rounded-lg border border-gray-300">
                          <Clock className="w-4 h-4 text-gray-600" />
                          {isPaused && timerShouldRun && isAnalysisRunning ? (
                            <>
                              <div className="w-1.5 h-1.5 bg-amber-500 rounded-full"></div>
                              <span className="text-amber-600 font-mono text-sm font-semibold min-w-[2.5rem]">
                                {formatElapsedTime(elapsedTime)}
                              </span>
                            </>
                          ) : timerShouldRun && isAnalysisRunning ? (
                            <>
                              <div className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse"></div>
                              <span className="text-rice-primary font-mono text-sm font-semibold min-w-[2.5rem]">
                                {formatElapsedTime(elapsedTime)}
                              </span>
                            </>
                          ) : timerShouldRun ? (
                            <>
                              ...
                              <span className="text-blue-600 font-mono text-sm font-semibold min-w-[2.5rem]">
                                {formatElapsedTime(elapsedTime)}
                              </span>
                            </>
                          ) : elapsedTime > 0 ? (
                            <span className="text-gray-600 font-mono text-sm font-semibold min-w-[2.5rem]">
                              {formatElapsedTime(elapsedTime)}
                            </span>
                          ) : (
                            <span className="text-gray-400 font-mono text-sm font-semibold min-w-[2.5rem]">
                              00:00
                            </span>
                          )}
                        </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between pt-4 mt-4 border-t">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 mt-4 sm:mt-0">
                      ...
                      {!isTmaEnabled() && completedSamples.length >= totalSamplesCount && (
                        <Button
                          onClick={() => onComplete?.()}
                          className="bg-blue-600 text-white hover:bg-blue-700 font-semibold shadow-md animate-pulse"
                        >
                          All Trials Complete - Proceed to Reports
                        </Button>
                      )}
                    </div>
                  </div>
          ...
          <div className="space-y-6">
                  ...
                  <CardContent>
                    <div className="relative">
                      ...
                      {webrtcConnected ? (
                        ...
                        // Error state
                        <div className="w-full bg-gradient-to-br from-red-800 to-red-900 flex items-center justify-center rounded-lg" style={{ aspectRatio: '16/9', minHeight: '420px' }} data-placeholder>
                        ...
                        <div className="w-full bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center rounded-lg" style={{ aspectRatio: '16/9', minHeight: '420px' }} data-placeholder>
                          <div className="text-center text-white">
            ...
            <div className="lg:col-span-1 min-w-0 space-y-6">
              ...
              <Card className="animate-fade-in h-fit shadow-md w-full border-l-4 border-l-rice-primary">
                ...
                <CardContent>
                  <div className="space-y-2 text-base mt-4">
                    ...
                    <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                      <span className="font-semibold text-gray-600">Frame Rate</span>
                      <span className="font-medium">
                        {isAnalysisRunning && cameraFps > 0 ? `${cameraFps} FPS` : '-- FPS'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                      <span className="font-semibold text-gray-600">Detections/Frame</span>
                      <span className="font-medium">
              ...
              {/* Summary Card */}
              <Card className="animate-fade-in h-fit shadow-lg w-full">
                  <CardContent className="p-4 sm:p-6 w-full">
                    {(() => {
                      const regionKey = millRegion === 'basmati' ? 'basmati' : 'non-basmati';
                      const categories = segConfig?.[regionKey]?.categories || segConfig?.[regionKey] || [];
                      const headKeys = (categories as any[]).filter((c: any) => c.group === 'headRice').map((c: any) => segKeyToMetricField[c.key]).filter(Boolean);
                      const brokenKeys = (categories as any[]).filter((c: any) => c.group === 'brokens').map((c: any) => segKeyToMetricField[c.key]).filter(Boolean);
                      const hrFields = headKeys.length > 0 ? headKeys : (millRegion === 'basmati'
                        ? ['headRice', 'secondOne', 'tibar'] as (keyof typeof metrics.goodRice)[]
                        : ['headRice', 'threeFourthHead'] as (keyof typeof metrics.goodRice)[]);
                      const brFields = brokenKeys.length > 0 ? brokenKeys : (millRegion === 'basmati'
                        ? ['dubar', 'miniDubar', 'mongra', 'miniMongra', 'nakku'] as (keyof typeof metrics.goodRice)[]
                        : ['halfBrokens', 'quarterFineBrokens', 'tips'] as (keyof typeof metrics.goodRice)[]);

                      const headRicePct = hrFields.reduce((sum, k) => sum + (metrics.goodRice[k] || 0), 0);
                      const headRiceCount = Math.round(hrFields.reduce((sum, k) => sum + (actualCounts.goodRice[k] || 0), 0));
                      const brokenPct = brFields.reduce((sum, k) => sum + (metrics.goodRice[k] || 0), 0);
                      const brokenCount = Math.round(brFields.reduce((sum, k) => sum + (actualCounts.goodRice[k] || 0), 0));
                      const chalkyPct = metrics.rejections.harvest.chalkyBellyCore || 0;
                      const chalkyCount = Math.round(actualCounts.rejections.harvest.chalkyBellyCore || 0);
                      const rejectsPct = (metrics.rejections.harvest.yellow || 0) +
                        (metrics.rejections.harvest.immatureGreen || 0) +
                        (metrics.rejections.harvest.peckyGrains || 0) +
                        (metrics.rejections.harvest.discolored || 0);
                      const rejectsCount = Math.round(
                        (actualCounts.rejections.harvest.yellow || 0) +
                        (actualCounts.rejections.harvest.immatureGreen || 0) +
                        (actualCounts.rejections.harvest.peckyGrains || 0) +
                        (actualCounts.rejections.harvest.discolored || 0)
                      );

                      const bars = [
                        { label: 'Head Rice', pct: headRicePct, count: headRiceCount, color: 'green' },
                        { label: 'Broken', pct: brokenPct, count: brokenCount, color: 'amber' },
                        { label: 'Chalky', pct: chalkyPct, count: chalkyCount, color: 'orange' },
                        { label: 'Rejections', pct: rejectsPct, count: rejectsCount, color: 'red' },
                      ];

                      return (
                    <div className="space-y-3">
                          {bars.map((bar) => {
                            const displayValue = showPercentage ? bar.pct : bar.count;
                            const maxValue = showPercentage ? 100 : 500;
                            const progressValue = (displayValue / maxValue) * 100;
                            ...
                            return (
                              <div key={bar.label} className="bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-200">
                                <div className="flex justify-between items-center mb-2 sm:mb-3">
                                  <span className={`text-xs sm:text-sm font-medium ${colors.text}`}>{bar.label}</span>
                                  <div className="flex items-center space-x-1 sm:space-x-2">
                                    ...
                                    <span className={`text-xs sm:text-sm font-bold ${colors.text}`}>{displayText}</span>
                                  </div>
                                </div>
                                <div className="w-full bg-gray-200 rounded-full h-2 sm:h-2.5">
                                  <div className={`rounded-full ${colors.bar} transition-all duration-500 ease-in-out`} style={{ width: `${Math.min(progressValue, 100)}%`, height: '100%' }}></div>
                                </div>
                              </div>
                            );
                          })}
                        ...
                        {completedSamples.length >= totalSamplesCount && (
                          <div className="space-y-3">
                            <div className="p-4 bg-green-50 border border-green-200 rounded-xl text-center">
                              ...
                              {analysisData?.isProductionSeries && analysisData?.seriesMachines && (
                                <p className="text-xs text-green-600 mt-1">
                                  Click below to proceed to the next machine in the series.
                                </p>
                              )}
                            </div>
                            <Button
                              onClick={() => onComplete?.()}
                              className="w-full bg-blue-600 text-white hover:bg-blue-700 font-semibold text-base py-6 shadow-md"
                              size="lg"
                            >
                              {analysisData?.isProductionSeries
