...
import { LiveTimeline } from "@/components/ios/live/LiveTimeline";
...
import { ProcessFlowRibbon, FlowStage } from "@/components/ios/ProcessFlowRibbon";
...
const ProcurementLiveAnalysis = ({ embedded = false, analysisDataOverride, onComplete }: LiveAnalysisProps = {}) => {
    ...
    if (currentSample < totalSamplesCount) {
      ...
      if (isTmaEnabled()) {
        ...
        if (isLastMachine) {
          ...
          // Auto-transition to insights & reports
          setTimeout(() => {
            console.log("🔄 Calling onComplete for reports transition...");
  ...
  if (!isClassicTheme) {
    ...
    const showCompleteButton = !isAnalysisRunning && allAnalysisDone;

    // Active stage on the process-flow ribbon. We don't have real telemetry
    // per-stage, but we can map session state heuristically: not running → null,
    // running with no whiteness yet → husking, running with whiteness → polishing.
    const activeFlowStage: FlowStage | null = !isAnalysisRunning
      ? null
      : (metrics.qualityIndices?.whitenessIndex ?? 0) > 0
        ? "polishing"
        : (gaugeValues.headRice ?? 0) > 0
          ? "whitening"
          : "husking";
    ...
    const composite: [RingDef, RingDef, RingDef] = [
      {
        key: "wi",
        label: "Whiteness",
        value: metrics.qualityIndices?.whitenessIndex ?? 0,
        max: 50,
        tokenVar: "--accent",
        unit: "WI",
        subtitle: "Target ≥ 45",
      ...
      },
      {
        key: "throughput",
        label: "Throughput",
        value: Math.min(100, (totalGrainsValue / 5000) * 100),
        max: 100,
        tokenVar: "--ios-orange",
        unit: "%",
        subtitle: `${totalGrainsValue.toLocaleString()} grains`,
      },
    ];
    ...
    return (
      <div className={embedded ? "w-full" : "flex-1 flex flex-col overflow-hidden ios-canvas"}>
        ...
        <div className={embedded ? "w-full p-4 sm:p-6 space-y-6" : "flex-1 overflow-auto p-6 space-y-6"}>
          ...
          {/* Process-flow ribbon — domain identity, glows on the active stage */}
          <ProcessFlowRibbon activeStage={activeFlowStage} />
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1.4fr_1fr] gap-5">
            <div className="space-y-5">
              ...
              {/* 60-second rolling timeline */}
              <LiveTimeline data={chartData} windowSec={60} />
            </div>
          ...
          {/* Bottom controls */}
          <div className="ios-surface border ios-hairline rounded-[18px] p-4 sticky bottom-4 z-10" style={{ backdropFilter: "blur(12px)" }}>
            <LivePillControls
              isRunning={isAnalysisRunning}
              isPaused={isPaused}
              isLoading={isLoading}
